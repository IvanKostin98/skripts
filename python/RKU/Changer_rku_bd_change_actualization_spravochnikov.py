...
#Переменные
path_to_rku_excel = r'\\***\Для всех\***\Pricing\Рита\***\РКУ 2024.xlsx'
...
df.rename(columns = {'ДТ с ндс':'ДТ_с_ндс_стандарт', 'ТТсндс':'ТТ_с_ндс_стандарт', 'МТсндс':'МТ_с_ндс_стандарт',
                      ...
                      'Название SKU':'SKU_name', 'BI название ДТ':'BI_name', 'Название сегмента':'Название_сегмента'
                      }, inplace = True)
...
for row in tqdm(df.itertuples(index=True)):
    try:
        cur.execute(f"""update [FinanceAndSAP].[segment].[Agreements]
                        set [Division] = '{row.Дивизион}', [SegmentName] = '{row.Название_сегмента}',
                        [Brand] = '{row.Бренд}', [SKU] = '{row.SKU_name}'
                        where year([TillDate]) = '2024' and [SKU_ID] = {int(row.Код_SKU)} and SegmentCode = '{row.Код_сегмента}'
                            """)
    except:
        counter+=1
